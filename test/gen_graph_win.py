...
def plot_rtt(time_list, SampleRTT, EstimatedRTT, DeviationRTT, TimeoutInterval):
    print("RTTsize:{0}".format(len(SampleRTT)))
        ...
        for num, line in enumerate(f):
            ...
            if line_list[1] == 'SEND':
                ...
                if line_list[0] not in RTTS_dic['utctime']: 
                    ...
                    # print("This is RTTS \n")
                    # print("data:{0} {1} {2} {3}".format(line_list[0],info_list[0],info_list[1],info_list[2],info_list[3]))
...
if len(CWND_dic['utctime']):
    ...
    plot_win(CWND_dic['time'][::intv], np.array(CWND_dic['size'][::intv]), 'Congestion', CWND_dic['type'][::intv]) # 仅绘制第100到第200的数据点
